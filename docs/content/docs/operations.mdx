...
The day-2 reference for running Chimely. For deployment and configuration,
see [Self-hosting](/docs/self-hosting).

Every guarantee here is enforced by the chaos test suite
(`server/tests/chaos.rs`): killing any process at any moment, losing Redis
entirely, flooding one environment, or replaying any job must not lose data,
drift a counter, or starve a neighbor.
...
chimely dlq replay --all             # replay every parked job
chimely dlq replay --all --env prod  # ...or scope to one environment slug
```
