...
* **Subscriber plane** — called by `@chimely/client` (the `<Inbox />`
  ...
  hash computed by the customer's backend:
  `hex(HMAC-SHA256(environment.subscriber_hmac_secret, env_typeid || 0x00 || subscriber_id))`
  where `env_typeid` is the environment's `env_…` TypeID. The legacy form
  without the environment binding,
  `hex(HMAC-SHA256(environment.subscriber_hmac_secret, subscriber_id))`,
  stays accepted while `CHIMELY_SUBSCRIBER_HASH_LEGACY_ACCEPT` is true (the
  default). Mandatory in environments with `require_subscriber_hash = true`
  (the production default); optional in dev environments so the quickstart
